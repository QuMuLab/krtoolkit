...
def unit_prop(theory):
    ...
    for cls in theory.clauses:
        if 1 == len(cls.literals):
            literals.add(cls.literals[0])
        else:
    ...
    while not stable:
        ...
        for i in range(len(nonUnitClauses)):
            ...
            # Check for subsumption and resolution
            for lit in cls.literals:
                ...
                unlit = lit.copy()
                unlit.sign = unlit.sign * -1
                    
                if unlit in literals:
                    cls.removeVariable(unlit.num)
                    stable = False
    ...
    all_clauses = list(nonUnitClauses) + [Clause([lit]) for lit in literals]
    theory.clauses = all_clauses
    theory.num_clauses = len(all_clauses)
